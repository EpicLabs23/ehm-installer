...
curl -o "${TARGET_DIR}/${NEW_EHM_VERSION}_ubuntu_20.04.tar.gz" $URL

# Check if the download was successful
if [ $? -ne 0 ]; then
  echo "Failed to download ${NEW_EHM_VERSION}_ubuntu_20.04.tar.gz"
  exit 1
fi

# Extract the file
echo "Extracting ${NEW_EHM_VERSION}_ubuntu_20.04.tar.gz to $TARGET_DIR..."
tar -xzvf "${TARGET_DIR}/${NEW_EHM_VERSION}_ubuntu_20.04.tar.gz" -C $TARGET_DIR

# Check if extraction was successful
if [ $? -ne 0 ]; then
  echo "Failed to extract ${NEW_EHM_VERSION}_ubuntu_20.04.tar.gz"
  exit 1
fi
